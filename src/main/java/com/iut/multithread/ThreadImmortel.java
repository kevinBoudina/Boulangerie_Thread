...
public class ThreadImmortel extends Thread {
    /*---------------------- ThreadImmortel ---------------------------------*/
...
//int j;
    ...
    public ThreadImmortel(String nom, Compte compte, int i) throws Exception {
        super(nom);
        this.compte = compte;
...
//------------------ run() -------------------------------------
        try {
            ...
            while (!this.isInterrupted()) // la boucle s'arrête seulement sur une demande d'interruption
            {
                ++compte.t[i];
                ++j;
                System.out.println(" le thread immortel courant : " + this.getName() + " accède au compte courant pour la " + j + "ème fois");
